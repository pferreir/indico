    ...
    def _process(self):
        ...
        action_menu_items = [
            ActionMenuEntry(
                'ajax-dialog',
                'mail',
                url_for('.email_registrants', reg_form),
                _('E-mail'),
                _('Send e-mail'),
                weight=100,
                hide_if_locked=True,
                ...
                'ajax-dialog',
                'attachment',
                url_for('.registrations_attachments_export', reg_form),
        ...
        if any(tpl.is_ticket for tpl in badge_templates):
            action_menu_items.append(ActionMenuEntry(
                'ajax-dialog',
                'id-badge',
                url_for('.registrations_config_badges', reg_form),
                _('Print Badges'),
                _('Print Badges'),
                weight=90,
            ))
        ...
        if any(not tpl.is_ticket for tpl in badge_templates):
            action_menu_items.append(ActionMenuEntry(
                'ajax-dialog',
                'ticket',
                url_for('.registrations_config_tickets', reg_form),
                _('Print Tickets'),
                _('Print Tickets'),
                weight=80,
            ))
